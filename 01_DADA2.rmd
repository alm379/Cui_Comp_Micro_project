...
# Don't forget to install pacman and DT if you don't have it yet. :)git
...
raw_fastqs_path <- "/workdir/alm379/git_repos/Cui_Comp_Micro_project/data/raw_data/raw_fastq_gz"
raw_fastqs_path
...
The data appears to be of high generally quality with most quality scores between 30 and 40.
...
The sequences are even higher quality, with the last 3 bases trimmed to remove the low quality read, 247. The Phred scores, which were above 30 to begin with, are even higher now.
...
86.33% of reads were retained post-filtering (2827430 reads retained), which seems to be enough reads to assign taxonomy. This level of read retention indicates that the initial data was of relatively high quality and that the filterAndTrim commands were effective at removing the lowest quality reads. As mentioned above, the "trimRight" parameter was set to "3" to ensure the removal of low quality read number 247.
...
The error rates for each possible transition (A→C, A→G, …) are shown in the plot above. The graphs estimate generally higher error rates than expected under the nominal definition of the Q-score. The error rates generally decrease with quality score, but increase sharply at the highest quality score (far right of the graphs).
